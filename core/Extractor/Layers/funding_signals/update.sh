...
# 確保分類子目錄存在
CATEGORIES=("funding_round" "ipo" "acquisition" "earnings_signal" "market_trend")

for category in "${CATEGORIES[@]}"; do
  mkdir -p "$DOCS_DIR/$category"
done

echo "=== funding_signals 資料更新開始 ==="
echo ""
...
⚠️ 注意：本 Layer 涉及財務資訊，請確保不構成投資建議。" 2>/dev/null || echo "警告: GitHub Issue 建立失敗" >&2
  fi
else
  echo "所有項目皆通過自動審核"
fi
