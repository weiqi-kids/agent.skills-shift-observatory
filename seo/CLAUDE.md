...
## 附錄 B：Skills Shift Observatory 整合說明

本站已整合 SEO 規則庫，採用 **方案 A+C** 架構：

### 整合架構

```
Mode CLAUDE.md（定義輸出規則）
        ↓
報告 .md 檔（含 seo front matter）
        ↓
Jekyll 模板（_includes/seo-schema.html）
        ↓
最終 HTML（含 JSON-LD）
```
...
| 文件 | 說明 |
|------|------|
| `seo/integration-design.md` | 整合設計文件 |
| `seo/front-matter-spec.md` | Front matter SEO 欄位規範 |
| `core/Narrator/seo-integration.md` | Narrator Mode 整合指引 |
| `_data/seo.yml` | 全站 SEO 設定 |
| `_includes/seo-schema.html` | JSON-LD 生成器 |
| `_includes/head_custom.html` | 自定義 head 內容 |
...
|--------|------|------|
| WebSite | ✅ 自動 | 全站設定 |
| Organization | ✅ 自動 | 全站設定 |
| Person | ✅ 自動 | 預設作者 |
| WebPage | ✅ 自動 | 所有頁面 |
| Article | ✅ 自動 | 有 `mode` 的報告 |
| BreadcrumbList | ✅ 自動 | 根據 `parent` 計算 |
| ImageObject | ✅ 自動 | 預設 OG 圖片 |
| FAQPage | ⚡ 按需 | 需 `seo.faq` 欄位 |
...
報告 front matter 必須包含：

```yaml
seo:
  title: "{SEO 標題，≤60字}"
  description: "{Meta 描述，≤155字}"
  keywords:
    - {關鍵字1}
    - {關鍵字2}
    ...
...
詳細規範請參閱 `seo/front-matter-spec.md`。
